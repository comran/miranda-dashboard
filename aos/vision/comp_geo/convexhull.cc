...
Vector<2> nextToTop(std::stack<Vector<2>> &S) {
  Vector<2> p = S.top();
  S.pop();
  Vector<2> res = S.top();
  S.push(p);
...
// between p1 and p2
int distSq(Vector<2> p1, Vector<2> p2) {
...
// 1 --> Clockwise
// 2 --> Counterclockwise
int orientation(Vector<2> p, Vector<2> q, Vector<2> r) {
...
int compare(const void *vp1, const void *vp2) {
  Vector<2> *p1 = (Vector<2> *)vp1;
  Vector<2> *p2 = (Vector<2> *)vp2;
...
std::vector<Vector<2>> convexhull(std::vector<Vector<2>> points) {
  ...
  // direction) than p1
  p0 = points[0];
  qsort(&points[1], n - 1, sizeof(Vector<2>), compare);
